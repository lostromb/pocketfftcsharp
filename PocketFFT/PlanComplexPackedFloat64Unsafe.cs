...
namespace PocketFFT
{
    internal class PlanComplexPackedFloat64Unsafe : IComplex1DFFTPlanFloat64
    {
        internal int length;
        internal int nfct;
        internal cmplx[] mem;
...
#else
        internal cfftp_fctdata_double[] fct; // [Constants.NFCT]
#endif

        public int Length => length;

        public PlanComplexPackedFloat64Unsafe(int length)
        {
            if (length == 0)
            {
            ...
            {
                this.fct = new cfftp_fctdata_double[Constants.NFCT];
            }
#endif
        ...
        private unsafe void pass_all(cmplx* c, double fct, int sign)
        {
            if (this.length == 1)
            {
                return;
            ...
            fixed (cmplx* ch = scratchArray)
            fixed (cmplx* memPtr = this.mem)
            {
                cmplx* p1 = c;
                cmplx* p2 = ch;
                ...
                for (int k1 = 0; k1 < nf; k1++)
                {
                    ...
                    if (ip == 4)
                    {
                        if (sign > 0)
                        {
                            pass4b(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                        else
                        {
                            pass4f(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                    }
                    else if (ip == 2)
                    {
                        if (sign > 0)
                        {
                            pass2b(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                        else
                        {
                            pass2f(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                    }
                    else if (ip == 3)
                    {
                        if (sign > 0)
                        {
                            pass3b(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                        else
                        {
                            pass3f(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                    }
                    else if (ip == 5)
                    {
                        if (sign > 0)
                        {
                            pass5b(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                        else
                        {
                            pass5f(ido, l1, p1, p2, memPtr + this.fct[k1].tw);
                        }
                    }
                    else if (ip == 7)
                    {
                        pass7(ido, l1, p1, p2, memPtr + this.fct[k1].tw, sign);
                    }
                    else if (ip == 11)
                    {
                        pass11(ido, l1, p1, p2, memPtr + this.fct[k1].tw, sign);
                    }
                    else
                    {
                        passg(ido, ip, l1, p1, p2, memPtr + this.fct[k1].tw, memPtr + this.fct[k1].tws, sign);
                        {
                            cmplx* tmp = p1;
                            p1 = p2;
                            p2 = tmp;
                ...
                if (p1 != c) // pointer address comparison
                {
                    if (fct != 1.0)
                    {
                        Intrinsics.ScalePointer(ch, c, len, fct);
                    }
                    else
                    {
                        Unsafe.CopyBlock(c, p1, (uint)(len * sizeof(cmplx)));
                    }
                }
                else
                {
                    if (fct != 1.0)
                    {
                        Intrinsics.ScalePointerInPlace(c, len, fct);
                    }
                }
            }
        ...
        private static unsafe void pass2b(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 2;
        ...
        private static unsafe void pass2f(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 2;
        ...
        private static unsafe void pass3b(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 3;
        ...
        private static unsafe void pass3f(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 3;
            const double tw1r = -0.5, tw1i = -0.86602540378443864676;
        ...
        private static unsafe void pass4b(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 4;
        ...
        private static unsafe void pass4f(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 4;
        ...
        private static unsafe void pass5b(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 5;
        ...
        private static unsafe void pass5f(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa)
        {
            const int cdim = 5;
            const double tw1r = 0.3090169943749474241,
                tw1i = -0.95105651629515357212,
        ...
        private static unsafe void pass7(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa, int sign)
        {
            const int cdim = 7;
        ...
        private static unsafe void pass11(int ido, int l1, cmplx* cc, cmplx* ch, cmplx* wa, int sign)
        {
            const int cdim = 11;
        ...
        private static unsafe void passg(int ido, int ip, int l1, cmplx* cc, cmplx* ch, cmplx* wa, cmplx* csarr, int sign)
        {
            int cdim = ip;
            int ipph = (ip + 1) / 2;
            int idl1 = ido * l1;
