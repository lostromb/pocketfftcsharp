...
NOINLINE static void pass3b (size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  if (ido==1)
    for (size_t k=0; k<l1; ++k)
      {
        cmplx t0 = CC(0, 0, k), t1, t2;
        PMC(t1, t2, CC(0, 1, k), CC(0, 2, k))
        CH(0, k, 0).r = t0.r + t1.r;
        CH(0, k, 0).i = t0.i + t1.i;
        cmplx ca, cb;
        ca.r = t0.r + tw1r * t1.r;
        ca.i = t0.i + tw1r * t1.i;
        cb.i = tw1i * t2.r;
        cb.r = -(tw1i * t2.i);
        PMC(CH(0, k, 1), CH(0, k, 2), ca, cb)
      }
  else
    for (size_t k=0; k<l1; ++k)
      {
      ...
      for (size_t i=1; i<ido; ++i)
        {
            ...
            cb.r = -(tw1i * t2.i);
            PMC(da, db, ca, cb)
            A_EQ_B_MUL_C(CH(i, k, 1), WA(1 - 1, i), da)
            A_EQ_B_MUL_C(CH(i, k, 2), WA(2 - 1, i), db)
        }
      }
  }
