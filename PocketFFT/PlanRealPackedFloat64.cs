...
namespace PocketFFT
{
    internal class PlanRealPackedFloat64 : IReal1DFFTPlanFloat64
    {
        internal struct rfftp_fctdata_double
        {
            public int fct;
            public int tw; // index into plan.mem
            public int tws; // index into plan.mem

            public rfftp_fctdata_double(int fct, int tw, int tws)
            {
                this.fct = fct;
                this.tw = tw;
                this.tws = tws;
        ...
        internal struct rfftp_fctdata_array_double
        {
            internal rfftp_fctdata_double data;
        }

        internal rfftp_fctdata_array_double fct;
#else
        internal rfftp_fctdata_double[] fct; // [Constants.NFCT]
#endif

        public int Length => length;

        public PlanRealPackedFloat64(int length)
        {
            if (length == 0)
            {
            ...
            {
                this.fct = new rfftp_fctdata_double[Constants.NFCT];
            }
#endif

            for (int i = 0; i < Constants.NFCT; ++i)
            {
                this.fct[i] = new rfftp_fctdata_double(0, 0, 0);
            }
        ...
        private static void rfftp_forward(PlanRealPackedFloat64 plan, Span<double> c, double fct)
        {
            if (plan.length == 1)
            {
                return;
        ...
        private static void rfftp_backward(PlanRealPackedFloat64 plan, Span<double> c, double fct)
        {
            if (plan.length == 1)
            {
                return;
